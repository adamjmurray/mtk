...
describe MTK::Sequencer::RhythmicSequencer do
  ...
  let(:rhythm)      { Pattern.RhythmSequence(0.0, 0.5, 1.5, 4) }
  let(:rhythmic_sequencer) { RHYTHMIC_SEQUENCER.new [pitches, durations, intensities, rhythm] }

  describe "#new" do
    it "defaults @max_times to nil" do
      rhythmic_sequencer.max_times.should be_nil
    end

    it "sets @max_times from the options hash" do
      rhythmic_sequencer = RHYTHMIC_SEQUENCER.new [], :max_times => 4
      rhythmic_sequencer.max_times.should == 4
    end
  end
