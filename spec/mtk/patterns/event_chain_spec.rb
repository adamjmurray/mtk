...
describe MTK::Patterns::EventChain do

  EVENT_CHAIN = MTK::Patterns::EventChain

  let(:pitch) { EVENT_CHAIN::DEFAULT_PITCH }
  let(:intensity) { EVENT_CHAIN::DEFAULT_INTENSITY }
  let(:duration)  { EVENT_CHAIN::DEFAULT_DURATION }
  ...
  describe "#new" do
    it "allows default pitch to be specified" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle(0)], :default_pitch => Gb4
      event_builder.next.should == [Note(Gb4, intensity, duration)]
    end
    it "allows default intensity to be specified" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle(0)], :default_intensity => ppp
      event_builder.next.should == [Note(pitch, ppp, duration)]
    end
    it "allows default duration to be specified" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle(0)], :default_duration => 5.25
  ...
  describe "#next" do
    ...
    it "defaults to a starting point of C4 (middle C)" do
      event_builder = EVENT_CHAIN.new [Patterns.Sequence(C4)]
      event_builder.next.should == notes(C4)
    end

    it "defaults to intensity 'o' when no intensities are given" do
      event_builder = EVENT_CHAIN.new [Patterns.NoteSequence(C4, D4, E4), Patterns.DurationCycle(2)]
    ...
    it "defaults to duration 1 when no durations are given" do
      event_builder =  EVENT_CHAIN.new [Patterns.NoteSequence(C4, D4, E4), Patterns.IntensityCycle(p,o)]
    ...
    it "builds notes from pitch class sets, selecting the neartest pitch classes to the previous/default pitch" do
      ...
      event_builder = EVENT_CHAIN.new [pitch_class_sequence], :default_pitch => D3
    ...
    it "builds notes from by adding Numeric intervals in :pitch type Patterns to the previous Pitch" do
      event_builder = EVENT_CHAIN.new [ Patterns.PitchSequence( C4, M3, m3, -P5) ]
      nexts = []
    ...
    it "builds notes from by adding Numeric intervals in :pitch type Patterns to all pitches in the previous Chord" do
      event_builder = EVENT_CHAIN.new [ Patterns.PitchSequence( Chord(C4,Eb4), M3, m3, -P5) ]
      nexts = []
      loop { nexts << event_builder.next }
    ...
    it "builds notes from intensities" do
      event_builder = EVENT_CHAIN.new [ Patterns.PitchCycle(C4), Patterns.IntensitySequence(mf, p, fff) ]
      nexts = []
      loop { nexts += event_builder.next }
    ...
    it "builds notes from durations" do
      event_builder = EVENT_CHAIN.new [ Patterns.PitchCycle(C4), Patterns.DurationSequence(1,2,3) ]
      nexts = []
      loop { nexts += event_builder.next }
    ...
    it "iterates through the pitch, intensity, and duration list in parallel to emit Notes" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle(C4, D4, E4), Patterns.IntensityCycle(p, o), Patterns.DurationCycle(1,2,3,4)]
      event_builder.next.should == [Note(C4, p, 1)]
      event_builder.next.should == [Note(D4, o, 2)]
    ...
    it "returns nil (for a rest) when it encounters a nil value" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle(C4, D4, E4, F4, nil), Patterns.IntensityCycle(mp, mf, o, nil), Patterns.DurationCycle(1, 2, nil)]
    ...
    it "goes to the nearest Pitch for any PitchClasses in the pitch list" do
      event_builder = EVENT_CHAIN.new [Patterns::PitchCycle(C4, F, C, G, C)]
      event_builder.next.should == notes(C4)
    ...
    it "does not endlessly ascend or descend when alternating between two pitch classes a tritone apart" do
      event_builder = EVENT_CHAIN.new [Patterns::PitchCycle(C4, Gb, C, Gb, C)]
      event_builder.next.should == notes(C4)
    ...
    it "handles pitches and chords intermixed" do
      event_builder = EVENT_CHAIN.new [Patterns.PitchCycle( Chord(C4, E4, G4), C4, Chord(D4, F4, A4) )]
    ...
    it "adds numeric intervals to Chord" do
      event_builder = EVENT_CHAIN.new [Patterns::PitchCycle( Chord(C4, E4, G4), 2 )]
      event_builder.next.should == notes(C4,E4,G4)
    ...
    it "goes to the nearest Pitch relative to the lowest note in the Chord for any PitchClasses in the pitch list" do
      event_builder = EVENT_CHAIN.new [Patterns::PitchCycle( Chord(C4, E4, G4), F, D, Bb )]
      event_builder.next.should == notes(C4,E4,G4)
      event_builder.next.should == notes(F4)
    ...
    it "uses the default_pitch when no pitch pattern is provided" do
      event_builder = EVENT_CHAIN.new [Patterns.IntensityCycle( mp, mf, o )], :default_pitch => G3
  ...
  describe "#rewind" do
    it "resets the state of the EventChain" do
      event_builder = EVENT_CHAIN.new [ Patterns.PitchSequence(C,P8) ]
